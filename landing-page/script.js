...
async function renderDashboardContent() {
  ...
  dashboardBody.innerHTML =
    '<div style="text-align:center;padding:24px;">Loading your dashboard...</div>';
  ...
  document
    .getElementById("dashboard-copy-code-btn")
    ?.addEventListener("click", copyAccessCode);
  document
    .getElementById("dashboard-download-extension-btn")
...
async function handleDashboardPlanSelect(plan, amount) {
  ...
  try {
    ...
    const response = await fetch(
      `${CONFIG.API_BASE_URL}/subscription/paystack/initialize`,
      {
        method: "POST",
        headers: {
...
async function verifyPaymentAndComplete(reference) {
  try {
    ...
    const response = await fetch(
      `${CONFIG.API_BASE_URL}/subscription/paystack/verify/${reference}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
function initBillingToggle() {
  ...
  const yearlyLabel = document.querySelector(
...
function initPricingButtons() {
  document.querySelectorAll("[data-plan]").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      const amountKey =
        currentBillingPeriod === "yearly" ? "amountYearly" : "amountMonthly";
...
function initModals() {
  ...
  document
    .getElementById("download-extension-btn")
    ?.addEventListener("click", function (e) {
      e.preventDefault();
      if (CONFIG.CHROME_EXTENSION_URL !== "#") {
        window.open(CONFIG.CHROME_EXTENSION_URL, "_blank");
...
async function generateAccessCode(accessToken) {
  try {
    const response = await fetch(
      `${CONFIG.API_BASE_URL}/subscription/access-codes`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
function showFormError(form, message) {
  ...
  errorDiv.style.cssText =
    "color: #ef4444; font-size: 0.875rem; text-align: center; padding: 12px; background: #fef2f2; border-radius: 8px; margin-bottom: 16px;";
