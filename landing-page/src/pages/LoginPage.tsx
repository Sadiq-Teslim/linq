...
// Messages for different logout reasons
const LOGOUT_MESSAGES: Record<string, string> = {
  session_expired: "Your session has expired. Please log in again.",
  token_expired: "Your session has expired. Please log in again.",
  logged_out_elsewhere: "You've been logged out from another tab or device.",
...
export const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (reason) {
      const message = LOGOUT_MESSAGES[reason] || LOGOUT_MESSAGES["session_expired"];
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                className="w-full px-4 py-3 rounded-lg glass bg-white/5 border border-white/10 text-white placeholder-slate-400 focus:outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/30 transition-all duration-200"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">
